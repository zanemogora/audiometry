...
//funguje mi to tak ze vygenerujem premennu computerChoice o nejakej hodnote a drzim hodnotu aj pri zmene vstupu pouzivatela, este mi treba if (generovanahodnota) then zapni konkretny zvuk 
    ...
    playbtn.addEventListener('click', function() {
       if (computerChoice == 'cervene_auto_sound') {
            var audio = new Audio('audio_prave_ucho/28_červené auto.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'cervena_sound') {
            var audio = new Audio('audio_prave_ucho/6_červená.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'jablko_jablcko_sound') {
            var audio = new Audio('audio_prave_ucho/46_jablko_jabĺčko.wav');
            audio.play();
            playFunc(audio, 1);
       }
