...
play_btn.addEventListener('click', function() {
       if (computerChoice == 'pes_psik_havo_sound') {
            var audio = new Audio('audio_lave_ucho/11a_pes_psík_havo.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'macka_a_pes_sound') {
            var audio = new Audio('audio_lave_ucho/30_mačka a pes.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'macka_a_konik_sound') {
            var audio = new Audio('audio_lave_ucho/32_mačka a koník.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'macka_macicka_sound') {
            var audio = new Audio('audio_lave_ucho/10_mačka_mačička.wav');
            audio.play();
            playFunc(audio, 1);
       }
})
