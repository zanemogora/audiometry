...
//funguje mi to tak ze vygenerujem premennu computerChoice o nejakej hodnote a drzim hodnotu aj pri zmene vstupu pouzivatela, este mi treba if (generovanahodnota) then zapni konkretny zvuk 
    ...
    play_btn.addEventListener('click', function() {
       if (computerChoice == 'auto_auticko_sound') {
            var audio = new Audio('audio_prave_ucho/2a_auto_autíčko.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'vlak_vlacik_sound') {
            var audio = new Audio('audio_prave_ucho/4_vlak_vláčik.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'banan_bananik_sound') {
            var audio = new Audio('audio_prave_ucho/47_banán_banánik.wav');
            audio.play();
            playFunc(audio, 1);
       }
