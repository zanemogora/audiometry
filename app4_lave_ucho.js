...
//funguje mi to tak ze vygenerujem premennu computerChoice o nejakej hodnote a drzim hodnotu aj pri zmene vstupu pouzivatela, este mi treba if (generovanahodnota) then zapni konkretny zvuk 
    ...
    play_btn.addEventListener('click', function() {
       ...
       if (computerChoice == 'bicykel_sound') {
            var audio = new Audio('audio_lave_ucho/1_bicykel.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'lietadlo_lietadielko_sound') {
            var audio = new Audio('audio_lave_ucho/3_lietadlo_lietadielko.wav');
            audio.play();
            playFunc(audio, 1);
       }

       if (computerChoice == 'autobus_sound') {
            var audio = new Audio('audio_lave_ucho/5_autobus.wav');
            audio.play();
            playFunc(audio, 1);
       }
    })
